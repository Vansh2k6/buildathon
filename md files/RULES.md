...
| **DET-3** | The fixture numbers in [TASKS.md §10](TASKS.md) are load-bearing. `BK-101` day 8 at `views 180 / orders 3` produces the ~60% drop the whole T1 beat depends on, and the 10-book table includes two titles (`BK-103`, `BK-107`) that exist solely to make rules fire. Do not adjust a number to "make the data look more realistic" |
